        ...
        <div className={`${style.outlier}`}>
            <div className={`${style.custom_sign_up_container}`}>
                    ...
                    <div className="col-sm-10">
                        {
                            loggedInUser.type === 'ADMIN' ?
                                (
                                    <select className="custom-select w-100 mr-1 mt-1 mt-sm-0"
                                            value={profileDetails.type} onChange={(event) => updateUserRole(event)}>
                                        <option value="ADMIN" selected={profileDetails.type === 'ADMIN'}>ADMIN</option>
                                        <option value="AUTHOR" selected={profileDetails.type === 'AUTHOR'}>AUTHOR
                                        </option>
                                        <option value="BROWSING"
                                                selected={profileDetails.type === 'BROWSING'}>BROWSING
                                        </option>
                                    </select>
                                ) : (
                                    <select className="custom-select w-100 mr-1 mt-1 mt-sm-0" value={profileDetails.type}
                                            onChange={(event) => updateUserRole(event)} disabled>
                                        <option value="ADMIN" selected={profileDetails.type === 'ADMIN'}>ADMIN</option>
                                        <option value="AUTHOR" selected={profileDetails.type === 'AUTHOR'}>AUTHOR</option>
                                        <option value="BROWSING" selected={profileDetails.type === 'BROWSING'}>BROWSING
                                        </option>
                                    </select>
                                )
                        }
                ...
                <div className={`form-group row ${style.remove_margin}`}>
                    ...
                    {!isClearEnabled && <button className={`btn btn-success col-sm ${style.margin_btn}`}
                                                onClick={(event) => updateProfile(event)} disabled>Update</button>}
                    ...
                    {isClearEnabled && <button className={`btn btn-outline-secondary col-sm ${style.margin_btn}`}
                                               onClick={(event) => clearAll(event)}>Clear Changes</button>}
                    {!isClearEnabled && <button className={`btn btn-outline-secondary col-sm ${style.margin_btn}`}
